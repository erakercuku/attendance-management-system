server <- function(input, output, session) {
  ...
  # Shared: Dynamically populate course dropdowns
  observe({
  ...
  output$attendance_trend_plot <- renderPlot({
  ...
  output$activity_heatmap_plot <- renderPlot({
  ...
  # ---- Tab 4: Role Distribution (Filtered) ----
  ...
  output$delay_trend_plot <- renderPlot({
  ...
  output$avgdelay_plot <- renderPlot({
    plot_avg_delay(
      schedule_df = module_schedule_df,
      attendance_df = user_actions_df,
      user_data_df = user_data_df,
  ...
  output$attendrank_table <- renderDataTable({
